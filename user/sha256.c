...
void print_hash(uint8_t hash[]) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        write(1, &high_nibble, 1); // Print high nibble
        write(1, &low_nibble, 1);  // Print low nibble
    }
    write(1, "\n", 1);
}
...
void sha256_final(SHA256_CTX *ctx, uint8_t hash[]) {
    ...
    // Append to the padded data the total message's length in bits and transform.
...
//     printf("%02x", hash[i]);   // hash is your 32-byte hash array
//     }
//     printf("\n");
//     exit(0);
// }
