...
//     uint32_t m[64], w[64], i, j, temp1, temp2, a, b, c, d, e, f, g, h;

//     for (i = 0; i < 16; ++i)
//         m[i] = (data[i * 4] << 24) | (data[i * 4 + 1] << 16) | (data[i * 4 + 2] << 8) | (data[i * 4 + 3]);
//     for (; i < 64; ++i)
//         m[i] = (m[i - 16] + ((m[i - 15] >> 7) | (m[i - 15] << 25)) + ((m[i - 15] >> 18) | (m[i - 15] << 14)) + (m[i - 15] >> 3)) +
...
//     for (i = 0; i < 64; ++i) {
//         temp1 = h + ((e >> 6) | (e << 26)) + ((e >> 11) | (e << 21)) + ((e >> 25) | (e << 7)) + (f ^ (e & (g ^ f))) + k[i] + m[i];
//         temp2 = ((a >> 2) | (a << 30)) + ((a >> 13) | (a << 19)) + ((a >> 22) | (a << 10)) + ((b & c) | (d & (b | c)));
//         h = g;
//         g = f;
//         f = e;
//         e = d + temp1;
...
//     for (int i = 0; i < 4; ++i)
//         for (int j = 0; j < 8; ++j)
//             hash[i + j * 4] = (ctx->state[j] >> ((3 - i) * 8)) & 0xff;
// }
